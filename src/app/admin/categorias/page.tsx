...
function CategoryModal({ isOpen, onClose, onSave, categoryToEdit }) {
        ...
        <div className={`fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-50 ${isOpen ? '' : 'hidden'}`}>
            <div className="bg-white p-8 rounded-lg w-1/3">
                <div className="text-center mb-4">
                    <h2 className="text-xl font-bold">{categoryToEdit ? 'Editar Categoría' : 'Añadir Categoría'}</h2>
                </div>
                <input
                    type="text"
                ...
                <div className="flex justify-center">
                    <button className="bg-green-500 text-white px-4 py-2 rounded mr-2" onClick={handleSave}>{categoryToEdit ? 'Guardar' : 'Añadir'}</button>
...
function CategoryItem({ category, onEdit, onDelete }) {
    return (
        <div className="flex justify-between items-center border-b border-gray-200 py-2">
            ...
            <div>
                <button className="mr-2 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={() => onEdit(category)}>Editar</button>
                <button className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded" onClick={() => onDelete(category)}>Eliminar</button>
            </div>
        </div>
    );
}
        ...
        <div>
            ...
            <div className="mt-20 px-4 ">
                <div className="flex justify-between items-center px-4">
                    ...
                    <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded" onClick={() => setIsAddModalOpen(true)}>Añadir</button>
                </div>
                <div className="mb-4">
                    <CategoryModal
