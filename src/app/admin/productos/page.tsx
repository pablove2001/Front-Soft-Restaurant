...
function ProductModal({ isOpen, onClose, onSave, productToEdit }) {
    ...
    const handleSave = () => {
        onSave({ title, imageUrl, price, description });
        onClose();
    };
        ...
        <div className={`fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-800 bg-opacity-50 ${isOpen ? '' : 'hidden'}`}>
            <div className="bg-white p-8 rounded-lg w-1/3">
                <div className="text-center mb-4">
                    <h2 className="text-xl font-bold">{productToEdit ? 'Editar Producto' : 'Agregar Producto'}</h2>
                </div>
                <input
                    type="text"
                ...
                <div className="flex justify-center">
                    <button className="bg-green-500 text-white px-4 py-2 rounded mr-2" onClick={handleSave}>{productToEdit ? 'Guardar' : 'Agregar'}</button>
...
function ProductCard({ title, imageUrl, price, description, onEdit, onDelete }) {
    return (
...
export default function ProductPage() {
    const [products, setProducts] = useState([
        { id: "001", title: "Taco de pastor", imageUrl: "/tacos.png", price: 20, description: "Descripción del producto 1" },
        { id: "002", title: "Coca cola", imageUrl: "/coca.png", price: 30, description: "Descripción del producto 2" },
        { id: "003", title: "Pastel chocolate", imageUrl: "/pastel.png", price: 25, description: "Descripción del producto 3" },
    ]);
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);
    const [productToEditIndex, setProductToEditIndex] = useState(null);

    const handleAddProduct = ({ title, imageUrl, price, description }) => {
        const newProduct = { id: Date.now().toString(), title, imageUrl, price, description };
        setProducts([...products, newProduct]);
    };

    const handleEditProduct = ({ title, imageUrl, price, description }) => {
                ...
                <div className="mx-auto px-10 flex items-center justify-between h-16">
                    ...
                    <div className="flex items-center">
                        <div className="relative ">
                            <h1 className="text-white font-bold italic text-4xl">Administrador / Productos</h1>
                        </div>
                    </div>
                    <div className="flex items-center">
